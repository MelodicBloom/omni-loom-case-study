        ...
        {NODES.map((node, i) => (
          <React.Fragment key={node.id}>
            <button
              ...
              className={`px-3 py-2 rounded border text-xs font-mono transition-all duration-150 ${
                hovered === node.id
                  ? 'scale-105 shadow-lg'
                  : 'opacity-80'
              }`}
              style={{
                ...
                boxShadow: hovered === node.id ? `0 0 16px ${node.color}30` : undefined,
              }}
            >
              {node.label}
            </button>
            {i < NODES.length - 1 && (
              <span className="text-[#263244] text-sm">→</span>
            )}
          </React.Fragment>
        ))}
      </div>
